...
class SanitizePage(QWidget):
    def __init__(self) -> None:
        ...
        self.preset.addItems(["Standard", "JKN Safe Mode", "Custom"])
        self.metadata = QCheckBox("Clean metadata")
        self.result = QTextEdit()
        self.result.setReadOnly(True)
        self.drop_label = PdfDropLabel()
        self.drop_label.files_dropped.connect(self.sanitize_paths)
    ...
    def sanitize(self) -> None:
        ...
        try:
            ...
            rules = {"metadata"} if self.metadata.isChecked() else set()
            if preset_text == "Custom" and not rules:
                QMessageBox.warning(
                    self, "Sanitize failed", "Select at least one sanitization rule."
                )
                return
            res = sanitize_pdf(source, Path(output), preset=preset_text, rules=rules)  # type: ignore[arg-type]
    ...
    def sanitize_paths(self, files: list[Path]) -> None:
        if not files:
            return
        source = files[0]
        output, _ = QFileDialog.getSaveFileName(
